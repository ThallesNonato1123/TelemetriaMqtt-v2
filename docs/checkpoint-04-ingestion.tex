...
backend/
  ingestion/
    __init__.py
    transform.py       # lógica pura: payload -> InfluxPoint
...
python -m ingestion --host 127.0.0.1 --port 1883 --no-tls \
  --username telemetria_reader --password <senha-local-de-dev> --dry-run &

python -m mock_publisher --host 127.0.0.1 --port 1883 --no-tls \
  --username esp32_telemetria --password <senha-local-de-dev> --duration 1.5
\end{lstlisting}

...
# Dispositivos de telemetria (ESP32 real ou o mock publisher): só podem
# publicar no próprio tópico, nunca ler -- nem esse nem qualquer outro.
user esp32_telemetria
topic write telemetria/esp32/data

# Consumidores (script de ingestao, Grafana Live via plugin MQTT): só
# podem ler o tópico de telemetria, nunca publicar.
...
docker exec telemetria-mosquitto mosquitto_sub -h localhost -p 1883 \
  -u telemetria_reader -P <senha-local-de-dev> -t telemetria/esp32/data -C 3 &

python -m mock_publisher --host 127.0.0.1 --port 1883 --no-tls \
  --username esp32_telemetria --password <senha-local-de-dev> --duration 2
\end{lstlisting}
...
parser.add_argument(
    ...
    help="desliga TLS (broker de dev local não usa TLS; produção usa)",
)
...
if not args.no_tls:
    client.tls_set()
